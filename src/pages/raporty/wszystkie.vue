<template>
  <div class="padded-wide">
    ...
    <h1>Raport</h1>
    <h2>Lista wszystkich zgłoszeń, wraz z podziałami administracyjnymi.</h2>
    ...
    <client-only>
      <table>
        ...
        <tr v-for="row in data" :key="row.id">
          <td>
            {{row.gmina.name}} <small>({{row.gmina.code}})</small>
          </td>
          <td>
...
export default {
  ...
  async fetch() {
    this.data = await this.$axios.$get('report/allSightings');
  },
  methods: {
    ...
    async download() {
      const data = await this.$axios.$get('report/allSightings?format=csv');
